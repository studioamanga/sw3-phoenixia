...
void Init(void)
{
  ...
  clAvaMan::Create();
  logOut("  [OK] Manager d'Avatar initialisŽ\n");

  switch(Disp)
    {
	  case DISP_JEU :
		  logOut("  [DO] Initialisation en mode Jeu\n");
		  InitJ();
		  break;
	  case DISP_MENU :
		  logOut("  [DO] Initialisation en mode Menu\n");
		  InitM();
		  break;
    }

  // Chargement de la configuration
  logOut("  [DO] Chargement de la config\n");
  LoadCfg();
