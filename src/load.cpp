...
*************************************************************
**                                                         **
********* load.cpp **** auteur : Vincent Tourraine **********
**                                                         **
** Fonctions relatives au chargement des textures depuis   **
** les formats BMP et TGA.                                 **
**                                                         **
*************************************************************/


#define EXITLOAD {fclose(fichier);return -1;}
#define CTOI(C) (*(int*)&C)	//rÈcupËre en int un nombre pointÈ par un char*
...
//Lit le fichier et son header
	...
	if (fread(Header,1,0x36,fichier)!=0x36) EXITLOAD;
	if (Header[0]!='B' || Header[1]!='M')	EXITLOAD;
	if (CTOI(Header[0x1E])!=0)		EXITLOAD;
	if (CTOI(Header[0x1C])!=24)		EXITLOAD;

//RÈcupËre les infos du fichier
	DataPos			= CTOI(Header[0x0A]);
	DataSize		= CTOI(Header[0x22]);
//RÈcupËre les infos de l'image
...
unsigned int ChargerImage24Bits (const char* szFilename)
{
	...
	// On crÈÈ un objet de texture qui est identifiÈ par m_uiTexture.
	glGenTextures (1, &uiTexture);
	// Indique que l'on utilise l'objet de texture identifiÈ par m_uiTexture.
	glBindTexture (GL_TEXTURE_2D, uiTexture);
	// ParamÈtrage de l'objet de texture, on utilise un filtre linÈaire.
	glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	// ParamÈtrage de l'objet de texture, on utilise un filtre linÈaire.
	glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// Chargement des donnÈes de la texture. OpenGL copie en mÈmoire les donnÈes.
