...
void Import(void)
{
  ...
  for (int o=0 ; o<M->nb_surface ; o++)
      ...
      for(int p=0 ; p<M->mod[o]->nb_vertices ; p++)
	{
	  ...
	  if(rep=='y')
	    {
	      M->mod[o]->vertices[p]->text.x=M->mod[o]->vertices[p]->p.x*facte;
	      M->mod[o]->vertices[p]->text.y=M->mod[o]->vertices[p]->p.z*facte;
	    }
	  fwrite(&(M->mod[o]->vertices[p]->text.x),sizeof(float),1,fOut);
	  fwrite(&(M->mod[o]->vertices[p]->text.y),sizeof(float),1,fOut);
	}
	...
	for (int o=0 ; o<M->nb_surface ; o++)
	  {
		...
		if(input=='S')
		  {
		    if(inAxe=='X')
		      M->mod[o]->vertices[p]->p.x=-M->mod[o]->vertices[p]->p.x;
		    if(inAxe=='Y')
		      M->mod[o]->vertices[p]->p.y=-M->mod[o]->vertices[p]->p.y;
		    if(inAxe=='Z')
		      M->mod[o]->vertices[p]->p.z=-M->mod[o]->vertices[p]->p.z;
		  }
		if(input=='H')
		  {
		    if(inAxe=='X')
		      M->mod[o]->vertices[p]->p.x=M->mod[o]->vertices[p]->p.x*homothe;
		    if(inAxe=='Y')
		      M->mod[o]->vertices[p]->p.y=M->mod[o]->vertices[p]->p.y*homothe;
		    if(inAxe=='Z')
		      M->mod[o]->vertices[p]->p.z=M->mod[o]->vertices[p]->p.z*homothe;
		  }
		if(input=='T')
		  {
		    if(inAxe=='X')
